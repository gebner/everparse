...
let serialize p x = FStar.IndefiniteDescription.indefinite_description_ghost bytes (fun y -> p y == Some (x, Seq.length y))

let serializes_to_intro p x b = LowParse.Spec.Base.parse_injective p (serialize p x) b

let serialize_inj p x1 x2 = serializes_to_intro p x1 (serialize p x2)

let serializes_to_ret v = serializes_to_intro (parse_ret v) v Seq.empty

let serializes_to_filter p f x =
  parse_filter_eq p f (serialize p x);
  serializes_to_intro (parse_filter p f) x (serialize p x)
...
let serializes_to_dtuple2 p1 p2 x1 x2 =
  ...
  FStar.Seq.Properties.append_slices b1 b2;
  serializes_to_intro (parse_dtuple2 p1 p2) (| x1, x2 |) (Seq.append b1 b2)
...
let serializes_to_nondep_then p1 p2 x1 x2 =
  ...
  FStar.Seq.Properties.append_slices b1 b2;
  serializes_to_intro (nondep_then p1 p2) (x1, x2) (Seq.append b1 b2)
...
let serializes_to_u8 x =
  ...
  serializes_to_intro LowParse.Spec.Int.parse_u8 x (Endianness.n_to_be 1 (UInt8.v x))

let serializes_to_u16be x =
  LowParse.Spec.Int.parse_u16_spec (Endianness.n_to_be 2 (UInt16.v x));
  serializes_to_intro LowParse.Spec.Int.parse_u16 x (Endianness.n_to_be 2 (UInt16.v x))

let serializes_to_u32be x =
  LowParse.Spec.Int.parse_u32_spec (Endianness.n_to_be 4 (UInt32.v x));
  serializes_to_intro LowParse.Spec.Int.parse_u32 x (Endianness.n_to_be 4 (UInt32.v x))

let serializes_to_u64be x =
  LowParse.Spec.Int.parse_u64_spec (Endianness.n_to_be 8 (UInt64.v x));
  serializes_to_intro LowParse.Spec.Int.parse_u64 x (Endianness.n_to_be 8 (UInt64.v x))

let serializes_to_u16le x =
  LowParse.Spec.BoundedInt.parse_u16_le_spec (Endianness.n_to_le 2 (UInt16.v x));
  serializes_to_intro LowParse.Spec.BoundedInt.parse_u16_le x (Endianness.n_to_le 2 (UInt16.v x))

let serializes_to_u32le x =
  LowParse.Spec.BoundedInt.parse_u32_le_spec (Endianness.n_to_le 4 (UInt32.v x));
  serializes_to_intro LowParse.Spec.BoundedInt.parse_u32_le x (Endianness.n_to_le 4 (UInt32.v x))

let serializes_to_u64le x =
  LowParse.Spec.Int.parse_u64_le_spec (Endianness.n_to_le 8 (UInt64.v x));
  serializes_to_intro LowParse.Spec.Int.parse_u64_le x (Endianness.n_to_le 8 (UInt64.v x))

let serializes_to_fldata p sz x =
  serializes_to_intro (parse_fldata p sz) x (serialize p x)
...
let in_codomain_fldata p sz x =
  introduce in_codomain (parse_fldata p sz) x ==> in_codomain p x /\ Seq.length (serialize p x) == sz with _.
    serializes_to_intro p x (serialize (parse_fldata p sz) x);
  introduce in_codomain p x /\ Seq.length (serialize p x) == sz ==> in_codomain (parse_fldata p sz) x with _.
    serializes_to_fldata p sz x

let serializes_to_list_nil p =
  parse_list_eq p Seq.empty;
  serializes_to_intro (parse_list p) [] Seq.empty
...
let serializes_to_list_cons p x xs =
  ...
  serializes_to_intro (parse_list p) (x::xs) bs
...
let in_codomain_list_cons p x xs =
  ...
  begin
    ...
    serializes_to_intro p x (Seq.slice bs 0 n)
  end;
